<template>
  <div v-if="movies.length > 0" class="container h-screen">
    ...
    <template v-if="movieStore.movieSpecial.data">
      <a-modal :open="open" width="1000px" centered :title="movieStore.movieSpecial.data.movie.name"
        @cancel="handleCancel" :footer="null">
        <div>
          <div>
            <a-tabs v-model="activeKey" :tab-position="mode" :style="{ height: 'auto' }" @tabScroll="callback">
              <template v-if="Object.keys(movieStore.groupedShowtimesSpecial).length">
                <a-tab-pane v-for="(rooms, date) in movieStore.groupedShowtimesSpecial" :key="date"
                  :tab="formatDate(date)">
