          ...
          <form @submit.prevent="handleSubmit">
            <div class="row">
              <div class="col-md-12 mb-3">
                ...
                <small
                  v-if="useAuthStore().errors && useAuthStore().errors.password"
                  class="text-danger"
                >
                  {{ useAuthStore().errors.password }}
                </small>
              </div>

              <div class="col-md-12 mb-3">
                <label class="form-label">
                  <span class="text-danger">*</span>Xác nhận mật khẩu</label
                >
                <input
                  type="password"
                  class="form-control"
                ...
                <small
                  v-if="
                    useAuthStore().errors &&
                    useAuthStore().errors.password_confirm
                  "
                  class="text-danger"
                >
                  {{ useAuthStore().errors.password_confirm }}
...
<script setup>
const emit = defineEmits(["submit-form-reset"]);
